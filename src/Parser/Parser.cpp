...
auto Parser::parseTileset(const pugi::xml_node& tilesetNode) -> tl::expected<Tileset, std::string> {
    ...
    // Parse image
    if (auto imageNode = tilesetNode.child("image")) {
    ...
    // Parse properties
    if (auto propertiesNode = tilesetNode.child("properties")) {
...
auto Parser::parseLayer(const pugi::xml_node& layerNode) -> tl::expected<Layer, std::string> {
    ...
    // Parse properties
    if (auto propertiesNode = layerNode.child("properties")) {
        layer.properties = parseProperties(propertiesNode);
    }
    
    // Parse data
    if (auto dataNode = layerNode.child("data")) {
